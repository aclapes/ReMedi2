...
void BackgroundSubtractor<cv::BackgroundSubtractorMOG2, ColorFrame>::subtract(int v, ColorFrame& frame)
{
    cv::BackgroundSubtractorMOG2& subtractor = m_Subtractors[v];
    ColorFrame::Ptr pInputFrame = m_InputFrames[v];
    subtract(subtractor, pInputFrame, frame);
}

void BackgroundSubtractor<cv::BackgroundSubtractorMOG2, ColorFrame>::subtract(vector<ColorFrame::Ptr>& colorFgFrames)
{
	for (int v = 0; v < m_InputFrames.size(); v++)
        subtract( v, *(m_InputFrames[v]) );
}

void BackgroundSubtractor<cv::BackgroundSubtractorMOG2, ColorFrame>::subtract(cv::BackgroundSubtractorMOG2& subtractor, ColorFrame::Ptr colorFrame, ColorFrame& colorFgFrame)
{
    cv::Mat image;
    if (m_bShadowsModeling) image = colorFrame->get();
...
void BackgroundSubtractor<cv::BackgroundSubtractorMOG2, DepthFrame>::subtract(int v, DepthFrame& frame)
{
    subtract(m_Subtractors[v], m_InputFrames[v], frame);
}

void BackgroundSubtractor<cv::BackgroundSubtractorMOG2, DepthFrame>::subtract(vector<DepthFrame::Ptr>& depthFgFrames)
{
	for (int v = 0; v < m_InputFrames.size(); v++)
        subtract(v, *(depthFgFrames[v]) );
}

void BackgroundSubtractor<cv::BackgroundSubtractorMOG2, DepthFrame>::subtract(cv::BackgroundSubtractorMOG2& subtractor, DepthFrame::Ptr depthFrame, DepthFrame& depthFgFrame)
{
    cv::Mat depthMap;
    depthFrame->get().convertTo(depthMap, cv::DataType<float>::type);
    cv::Mat depthMask;
    
    subtractor(depthMap, depthMask, 0);
...
void BackgroundSubtractor<cv::BackgroundSubtractorMOG2, ColorDepthFrame>::subtract(vector<ColorDepthFrame::Ptr>& fgFrames)
{
    if ( m_Modality == 0 || m_Modality == 2 )
    {
        for (int v = 0; v < BackgroundSubtractor<cv::BackgroundSubtractorMOG2, ColorFrame>::getInputFrames().size(); v++)
            BackgroundSubtractor<cv::BackgroundSubtractorMOG2, ColorFrame>::subtract( v, *(fgFrames[v]) );
    }
    else if ( m_Modality == 1 )
    {
        for (int v = 0; v < BackgroundSubtractor<cv::BackgroundSubtractorMOG2, DepthFrame>::getInputFrames().size(); v++)
            BackgroundSubtractor<cv::BackgroundSubtractorMOG2, DepthFrame>::subtract( v, *(fgFrames[v]) );
    }
    else if ( m_Modality == 3 )
    {
        for (int v = 0; v < m_InputFrames.size(); v++)
...
void BackgroundSubtractor<cv::BackgroundSubtractorMOG2, ColorDepthFrame>::subtract(int v, ColorDepthFrame& frame)
{
    subtract(m_Subtractors[v], m_InputFrames[v], frame);
}

void BackgroundSubtractor<cv::BackgroundSubtractorMOG2, ColorDepthFrame>::subtract(cv::BackgroundSubtractorMOG2& subtractor, ColorDepthFrame::Ptr frame, ColorDepthFrame& fgFrame)
{
    cv::Mat colorImage, depthMap;
    frame->getColor().convertTo(colorImage, cv::DataType<float>::type);
    frame->getDepth().convertTo(depthMap, cv::DataType<float>::type);
